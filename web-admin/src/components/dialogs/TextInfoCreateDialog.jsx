...
const TextInfoCreateDialog = ({ showDialog, closeDialog, picture, context, picShow }) => {
    ...
    async function handleCreateTextInfo() {
        ...
        try {
            const { status } = await HttpService.post('/api/league/context/info', data1)
            if (status === 200) {
                toastSuccess("ثبت با موفقیت انجام شد")
                // window.location.reload()
                closeDialog()
                history.push('/dashbord/textInfo')
            }
        } catch (ex) {
...
export default TextInfoCreateDialog;